...
exports.EquipmentSupply= async (req, res) => {
  try {
    ...
    const [detail, total, agg, byStatus] = await Promise.all([
      ...
      EquipmentSupply.countDocuments(filter),
    ...
    res.status(500).json({ message: 'خطأ في تقرير مستلزمات المعدات', error: err.message });
  }
};
