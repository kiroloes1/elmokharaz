...
const customer = require('../../models/peapole/supplier');
...
exports.deleteSupplier = async (req, res) => {
  ...
  try {
    ...
    const payments = await paymentModel.find({ supplier: id });
    ...
    await  equipment.deleteMany(  
      { supplierId: id },
      { session })   
    await  EquipmentSupply.deleteMany(
        { supplierId: id },
         { session })
    ...
    await  maintenance.deleteMany( 
           { supplierId: id },
           { session })
    await  BagPurchase.deleteMany( 
           { supplierId: id },
           { session })
    await  wirePurchase.deleteMany( 
           { supplierId: id },
           { session })
  ...
  try {
    ...
    if (paymentMethod === "cash"){
        ...
        await Transaction.create([{
            type: "expense",
            note: note || "دفع فلوس نقدي  التاجر " + supplier.name,
            items: [{
                title: " دفع التاجر " + supplier.name,
                category: "supplier",
    ...
    paymentData.walletInfo = {
        ...
        formData={
                ...
                receiverPhone:req.body.walletInfo?.receiverPhone,
                type: 'send',
                notes:  `عمليه ارسال اموال الي التاجر ${supplier.name} سيستم المخرز`,
...
await createLog({
  ...
  targetId: supplier._id,
  title: supplier.name,
details: `تم دفع مبلغ ${Number(amount).toLocaleString()} ج.م إلى التاجر "${supplier.name}" عن طريق ${paymentMethodTranslation(paymentMethod)}. تم تحديث رصيد التاجر من ${Number(oldSupplier).toLocaleString()} ج.م إلى ${Number(supplier.balance).toLocaleString()} ج.م بتاريخ ${new Date(date || Date.now()).toLocaleDateString("ar-EG")}.`
,session,
});
  ...
  try {
    ...
    if (paymentMethod === "cash") {
        ...
        await Transaction.create([{
            ...
            items: [{
                title: "استلام فلوس من التاجر  " + supplier.name,
                category: "supplier",
                amount: Number(amount)
            }],
            supplierId: supplier._id,
    ...
    paymentData.walletInfo = {
        ...
        formData={
                ...
                receiverPhone:  req.body.walletInfo?.receiverPhone,
                type: 'receive',
                notes:  `عمليه استلام اموال من التاجر ${supplier.name} سيستم المخرز`,
                amount: Number(req.body.amount),
                createdAt:date,
        }
...
await createLog({
  section: "تجار مشتريات",
  action: "استلام مبلغ من التاجر",
  userId: req?.user?.userId,
  targetId: supplier._id,
  title: supplier.name,
  details: `تم استلام مبلغ ${Number(amount).toLocaleString()} ج.م من التاجر "${supplier.name}" عن طريق ${paymentMethodTranslation(paymentMethod)}. تغير الرصيد من ${Number(oldSupplier).toLocaleString()} ج.م إلى ${Number(supplier.balance).toLocaleString()} ج.م بتاريخ ${new Date(date || Date.now()).toLocaleString("ar-EG")}.`
,session,
});
...
//   let existChequePayment=null
//   if (!existPaymentHistory) {

     

//       existChequePayment=await paymentModel.findOne({cheque:paymentId}).session(session);

//       if(!existChequePayment){
//         await session.abortTransaction();
//       session.endSession();
...
//         type:
//     existPaymentHistory.module === "pay"
//         ? "expense" 
//         : "income"
//       }).session(session);
...
//   targetId: supplier._id,
//   title: supplier.name,
//   details: `تم حذف عملية ${existPaymentHistory.module === "pay" ? "دفع"  : "استلام"} بقيمة ${Number(existPaymentHistory.amount).toLocaleString()} ج.م كانت تمت عن طريق ${existPaymentHistory.paymentMethod}. تغير رصيد التاجر من ${Number(
//     existPaymentHistory.module === "pay"
...
exports.editPaymentHistory = async (req, res) => {
  ...
  if (!existPaymentHistory) {
    ...
    if (existPaymentHistory.module == "pay") {
...
await Transaction.create([{
    type: existPaymentHistory.module === "pay" ? "expense" : "income",
    note: existPaymentHistory.notes ||
          (existPaymentHistory.module === "pay"
              ? `دفع للتاجر ${supplier.name}`
              : `استلام من التاجر ${supplier.name}`),
    items: [{
        title: existPaymentHistory.module === "pay"
            ? `دفع التاجر ${supplier.name}`
            : `استلام من التاجر ${supplier.name}`,
        category: "supplier",
        amount: Number(existPaymentHistory.amount)
    }],
    supplierId: supplier._id,
    date: existPaymentHistory.transactionDate
}], { session });
    ...
    const formData = {
        ...
        notes:
            walletType === "receive"
                ? `عملية استلام أموال من التاجر ${supplier.name}`
                : `عملية إرسال أموال إلى التاجر ${supplier.name}`,
        amount: Number(existPaymentHistory.amount),
...
if (paymentMethod === "cheque") {
    ...
    const cheque = await Cheque.create([{
        ...
        moneyFlow:existPaymentHistory.module === "pay" ? "outgoing" : "incoming",
    }], { session });
...
await existPaymentHistory.save({ session });
  ...
  targetId: supplier._id,
  title: supplier.name,
  details: `تم تعديل عملية ${existPaymentHistory.module === "pay" ?"دفع" : "استلام" } للتاجر ${supplier.name}. المبلغ الجديد ${Number(existPaymentHistory.amount).toLocaleString()} ج.م، وطريقة الدفع ${existPaymentHistory.paymentMethod}. أصبح رصيد التاجر ${Number(supplier.balance).toLocaleString()} ج.م بتاريخ ${new Date().toLocaleString("ar-EG")}.`
,session,
});
