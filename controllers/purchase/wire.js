...
const wireModel=require(`${__dirname}/../../models/purchase/wire/wirePurchase`);
const wireTypeModel=require(`${__dirname}/../../models/purchase/wire/WireType`);
...
exports.createWire=async(req,res)=>{
    ...
    try{
        ...
        for (const item of items) {
            if (!item.wireType)
                throw new Error("نوع السلك مطلوب");

            if (!item.size?.trim())
                throw new Error(" مقاس السلك مطلوب");
            ...
            const wireTypeExists = await wireTypeModel
                .findById(item.wireType)
                .session(session);

            if (!wireTypeExists)
                throw new Error("نوع السلك غير موجود");
        ...
        const lastPurchase= await wireModel
        .findOne({
        ...
        const purchase = await wireModel.create([{
...
title:
`دفع نقدي للتاجر ${supplierExists.name} مقابل شراء أسلاك `,
                                    category: "wire",
                                    amount: Number(p.paidAmount)
                                },)   
                                }
                                ...
                                if (!p.walletInfo) {
                                 ...
                                 const  formData={
                                                    ...
                                                    receiverPhone:  p.walletInfo?.receiverPhone,
                                                    type: 'send',
                                                    notes:  `عمليه ارسال اموال الي التاجر ${supplierExists.name} مقابل شراء أسلاك سيستم المخرز`,
                                                    amount: Number(p.paidAmount),
            ...
            type: "expense",
            note: note || "دفع نقدي للتاجر " + supplierExists.name + " مقابل شراء أسلاك ",
            items: itemsUpdate || [],
...
await createLog({
    ...
    title: `فاتورة شراء أسلاك رقم ${purchase.invoiceNumber}`,
details: `
تم تعديل فاتورة شراء أسلاك رقم ${purchase.invoiceNumber}
...
`,
    session
            ...
            await session.commitTransaction();
                ...
                res.status(201).json({
                    message: "تم إنشاء فاتورة شراء الأسلاك بنجاح",
                    wirePurchase: purchase[0]
                });
...
exports.updateWire = async (req, res) => {
    ...
    try {
        ...
        const oldwirePurchase = await wireModel.findById(id).session(session);
        if (!oldwirePurchase) throw new Error("فاتورة شراء مستلزمات المعدات غير موجودة");

        // Determine target supplier (either newly selected or old one)
        const targetSupplierId = supplier || oldwirePurchase.supplier;

        const oldSupplier = await supplierModel.findById(oldwirePurchase.supplier).session(session);
        ...
        const oldPaid = oldwirePurchase.paidAmount || 0;
        const oldTotal = oldwirePurchase.totalAmount || 0;
        ...
        // 5. UPDATE EQUIPMENT
        const purchase = await wireModel.findByIdAndUpdate(
            id,
            {
        ...
        for (const p of payments) {
            ...
            if (p.paymentMethod === "cash") {
                itemsUpdate.push({
                    title: `دفع نقدي للمورد ${newSupplier.name} مقابل شراء أسلاك`,
                    category: "wire",
                    amount: Number(p.paidAmount)
                });
            }
            ...
            if (p.paymentMethod === "wallet") {
                ...
                const formData = {
                    ...
                    receiverPhone: p.walletInfo?.receiverPhone,
                    type: 'send',
                    notes: `عملية إرسال أموال إلى التاجر ${newSupplier.name} مقابل شراء أسلاك`,
                    amount: Number(p.paidAmount),
                    createdAt: purchaseDate,
                    ispay:false
                };
        ...
        if (itemsUpdate.length > 0) {
            await TransactionModel.create([{
                type: "expense",
                note: note || `دفع نقدي للتاجر ${newSupplier.name} مقابل شراء أسلاك`,
                items: itemsUpdate,
...
exports.deleteWire = async (req, res) => {
    ...
    try {
        ...
        const oldWirePurchase = await wireModel.findById(id).session(session);
        if (!oldWirePurchase)
            throw new Error("فاتورة شراء الأسلاك غير موجودة");

        // Supplier
        const supplier = await supplierModel
            .findById(oldWirePurchase.supplier)
            .session(session);
        ...
        const oldPaid = oldWirePurchase.paidAmount || 0;
        const oldNet = (oldWirePurchase.totalAmount || 0) - oldPaid;
        ...
        await createLog({
            ...
            targetId: oldWirePurchase._id,
            title: `فاتورة شراء أسلاك رقم ${oldWirePurchase.invoiceNumber}`,
            details: `
تم حذف فاتورة شراء الأسلاك رقم ${oldWirePurchase.invoiceNumber}
الخاصة بالتاجر ${supplier.name}
بتاريخ ${new Date(oldWirePurchase.purchaseDate).toLocaleDateString("ar-EG")}.

كانت قيمة الفاتورة ${Number(oldWirePurchase.totalAmount).toLocaleString()} ج.م،
وتم دفع ${Number(oldWirePurchase.paidAmount).toLocaleString()} ج.م.
...
`,
        ...
        // Delete Invoice
        await wireModel.findByIdAndDelete(id, { session });

        await session.commitTransaction();
        session.endSession();

        return res.status(200).json({
            message: "تم حذف فاتورة شراء الأسلاك بنجاح"
        });
...
// Get All Wire Purchases
exports.getAllWire = async (req, res) => {
    try {
        // Pagination
        ...
        // Wire Type
        if (wireType) {
            if (!mongoose.Types.ObjectId.isValid(wireType)) {
                return res.status(400).json({
                    message: "معرف نوع السلك غير صحيح"
                });
            }
        ...
        if (search) {
            ...
            const wireTypes = await wireTypeModel.find({
                name: {
                    $regex: search,
                    $options: "i"
                }
        ...
        // Count
        const total = await wireModel.countDocuments(filter);
...
// Get Wire Purchase By Id
exports.getWireById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!wire) {
            return res.status(404).json({
                message: "فاتورة شراء الأسلاك غير موجودة"
            });
        }
        ...
        wire.payments = filteredPayments;

        return res.status(200).json({
            message: "تم جلب بيانات فاتورة شراء الأسلاك",
            wire
        });
...
// Print Wire Invoice
exports.printWire = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        return res.status(200).json({
            message: "تم جلب بيانات طباعة فاتورة شراء الأسلاك بنجاح",
            wire
        });
...
// Get Wire Purchases By Supplier
exports.getWireBySupplier = async (req, res) => {
    try {
        ...
        const wireIds = wires.map(w => w._id);
       ...
       const filteredPayments = payments.filter(payment => {
        ...
        wires.forEach(wire => {
            wire.payments = filteredPayments.filter(
                p => p.moduleId.toString() === wire._id.toString()
            );
        });
